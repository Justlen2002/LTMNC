...
def run_tester(s,path):
    ...
    obj = models.submission.objects.get(id=s)
    try:
    ...
    def get(self, request):
        if request.user.is_active:
            ...
            try:
                ...
                allStatus = models.submission.objects.filter(candidate_id=request.user.id).order_by('-id')
                idContestSubmitted = allStatus.values_list('contest_id',flat=True)
...
class Standing(View):
    def get(self, request, id):
        ...
        try:
            ...
            userName = ''
        # Scoreboard
        status = models.submission.objects.filter(contest_id = id).order_by('-status','TimeSubmit')
        ...
        for x in status:
            ...
            tg = {
                'iduser' : x.id,
                'name' : User.objects.get(id = x.candidate_id).username,
                'time' : x.TimeSubmit,
